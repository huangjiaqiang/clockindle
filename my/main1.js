...
function updateWeather() {
    ...
    fetch('http://a.niube.top:8080/api/weather?city=350604&extensions=base', {
...
function updateTime() {
    ...
    var time = now.toLocaleTimeString('zh-CN', timeOptions).replace(/上午|下午/g, '');
    var isPM = now.getHours() >= 12;
    var periodText = isPM ? '下午' : '上午';
    
    document.getElementById('current-time').innerHTML = 
        '<span class="period-indicator">' + periodText + '</span>' + time;
    
    var dateOptions = { month: 'long', day: 'numeric' };
    var dateStr = now.toLocaleDateString('zh-CN', dateOptions).replace('月', '月 ');
